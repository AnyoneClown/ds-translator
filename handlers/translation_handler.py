"""Translation Command Handler - Single Responsibility Principle."""
...
class TranslationHandler:
    ...
    def register_events(self):
        ...
        async def on_message(message):
            ...
            if (
                message.content.startswith(self._bot.command_prefix)
    ...
    async def _handle_translate_to_language(self, ctx, target_language: str):
        ...
        try:
            ...
            async with ctx.typing():
                result = self._translation_service.translate_to_language(
            ...
            if result:
                ...
                await ctx.reply(
                    f"Sorry, I couldn't translate that to {target_language}."
                )
    ...
    async def _handle_translate_to_english(self, ctx):
        ...
        try:
            ...
            if result and result.get("language") != "English":
                ...
                await ctx.reply(
                    f"Translated from {result.get('language')}:\n> {translated_text}"
                )
            elif result:
    ...
    async def _handle_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingRole):
            await ctx.reply(
                "You do not have the required 'Translator' role to use this command."
            )
        else:
